...
function transformToLis(obj) {
  ...
  let arr = x.map((item, index) => {
    return `<li>${item}: ${y[index]}</li>`;
  });
  return arr;
}
...
This function should then raise 2 to the power of the xing numbers, returning an array of arrays.
  ...
  input.map((item, index) => {
    ...
    item.map((value) => {
      if (typeof value === "number" && value % 5 === 0) {
        ...
        arr[index].push(percentFiveValue);
      }
    });
  });
  return arr;
...
returns the names of the ys whose gender is either male or female.

The names should be combined into a single string with each y name separated by "and".
...
let findMaleAndFemale = (data) => {
  ...
  let arr = [];
  data.map((y) => {
    if (y.gender === "female" || y.gender === "male") {
      arr.push(y.name);
    }
  });
...
Write a function named findShortest that, given the Star Wars data from Challenge 6, uses any combination of filter, map and reduce to return the name of the y who is the shortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {
  let arr = data[0];
  data.map((item) => {
    if (parseInt(item.height) <= parseInt(arr.height)) {
      arr = item;
    }
  });
  return arr.name;
...
describe("Testing challenge 4", () => {
  test("It should return numbers divisible by five, then raise two to the power of the xing numbers", () => {
    expect(
...
describe("Testing challenge 5", () => {
  test("It should return only ys that are male or female", () => {
...
describe("Testing challenge 6", () => {
  test("It should return the name of the shortest y", () => {
